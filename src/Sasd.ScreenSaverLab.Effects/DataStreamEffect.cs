...
/// <summary>
/// A calm data-stream effect with falling technical glyph streams.
/// </summary>
/// <remarks>
/// This effect is deliberately an original SASD-style implementation. It uses simple
/// ASCII-like technical glyphs and avoids copying any movie, vendor-specific visual style
/// or historic screensaver one-to-one. The goal for V0.2 is to prove that several built-in
/// effects can live behind the same host interface and can be selected by command-line option.
/// </remarks>
public sealed class DataStreamEffect : IScreenSaverEffect
{
    ...
    private readonly List<StreamColumn> _columns = [];
    private float _animationTime;

    /// <inheritdoc />
    public string Name => "Data Stream";

    /// <inheritdoc />
    public string Description => "A SASD data-stream effect with falling technical glyph streams.";
    ...
    public void Update(TimeSpan elapsed, Size viewportSize)
    {
        ...
        {
            StreamColumn column = _columns[index];
    ...
    /// <summary>
    /// Creates randomized movement state for one stream column.
    /// </summary>
    private StreamColumn CreateColumn(float x, Size viewportSize, bool randomizeVerticalPosition)
    {
    ...
    private static void DrawBackground(Graphics graphics, Size viewportSize)
    {
        ...
        // A subtle translucent veil prevents the data streams from looking too harsh on large
    ...
    private void DrawColumns(Graphics graphics, Size viewportSize)
    {
        ...
        foreach (StreamColumn column in _columns)
        {
    ...
    /// that the stream does not flicker aggressively.
    /// </summary>
    private char PickGlyph(StreamColumn column, int trailIndex)
    {
